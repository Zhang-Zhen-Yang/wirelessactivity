 ...
 * @Last Modified time: 2018-11-28 13:59:44
 */
// 选项卡
<template>
  <div class="tab">
    <table cellspacing="0" class="tab-title-warp">
      <tr>
        <td v-for="item,index in data.list"
          :class="['border-theme', 'tab-title', 'center', getActiveIndex() == index? 'tab-title-active' : 'tab-title-inactive']"
    ...
    >
      ...
      <table cellpadding="0" cellspacing="0" class="empty-content" v-if="content.children && content.children.length == 0">
        <tr>
      ...
      <div v-for="subItem,subIndex in content.children" >
        ...
        >
          <!--单列-->
          <!--<one-row  :data="subItem" :global="global" v-if="subItem.type=='oneRow'">
            daaadfadfadf{{ subIndex }}
          </one-row>-->
          <!--双列-->
          <!--<two-row :data="subItem" :global="global" v-if="subItem.type == 'twoRow'">
            daaadfadfadf{{ subIndex }}
          </two-row>-->
