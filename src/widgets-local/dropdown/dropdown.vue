...
<template>
  <div class="hsy-dropdown" :class="cls" :style="{width: width + 'px'}">
    <div class="selected" @click="autoShow" :style="selectedStyle" v-html="selectedText"><!--{{ selectedText }}--></div>
    <transition name="fadeIn" enter-active-class="animated fadeIn" leave-active-class="animated fadeOut">
      <div class="list scrollbar-overwrite" v-show="isShow" :style="{width: fixListWidth ? width + 'px' : 'auto', maxHeight: '400px',overflow: 'auto'}">
        <div class="inner">
          <div class="item" v-if="!grouped" v-for="item in items" :data-title="item.label" @click="!multiple && itemClicked(item)" :class="{selected: (item.label == selectedText)|| item.selected}">
            {{ appendIdx(item) }}
            <div v-if="multiple">
          ...
          <div v-if="grouped">
            <div v-for="group in items" class="group" style="position: relative;">
              <h3 class="group-label">{{ group.label }}</h3>
              <div class="sub-item-wrap">
                <div class="item sub-item" v-for="item in group.children" :data-title="item.label" @click="!multiple && itemClicked(item)" :class="{selected: item.selected}">
                  {{ appendIdx(item) }}
                  <div v-if="multiple">
                    <input type="checkbox" :disabled="item.disabled" :checked="item.selected" @change="checkboxChanged(item)" :id="id(item)">
                    <label :for="id(item)" @click="itemClicked(item)">{{ item.label }}</label>
